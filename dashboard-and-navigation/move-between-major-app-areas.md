# Move Between Major App Areas

Use the sidebar, section groups, quick links, and collapse control to move through the main SPRK pages.

## Purpose

Use this workflow when you want to understand how the app is organized and how to move from one major page to another.

## Prerequisites

- You are signed in to SPRK.
- A company is active in the sidebar.

## Steps

1. Use the left sidebar as the main navigation control.
2. Read the app in its current top-level groups:
   - `General`
   - `Accounting`
   - `System`
3. Select any page label in the expanded sidebar to open that area in the main panel.
4. If you want a narrower sidebar, use the collapse button at the top of the sidebar.
5. In collapsed mode, use the icon-only sidebar and hover or recognize the icons before selecting a destination.
6. If you are on the dashboard, use `Quick Links` for fast access to the linked pages that are currently exposed there.
7. When you need settings-style system work, use the `System` section rather than looking for a separate top navigation bar.

## Expected Result

You can move reliably between the app’s major work areas and understand whether you are navigating by full labels, icons, or dashboard quick links.

## Common Mistakes

- Looking for page tabs across the top of the app. Most page changes happen from the sidebar.
- Forgetting that the sidebar can be collapsed, which makes labels disappear and leaves icon-only navigation.
- Expecting the dashboard quick links to cover every page. They are shortcuts, not a full menu.
- Searching for a `Settings` sidebar item. System pages are exposed as specific destinations like `Companies`, `License`, `Support`, and `Preferences`.

## Related Articles

- [Understand the dashboard overview](./understand-the-dashboard-overview.md)
- [Use global search](./use-global-search.md)
- [Understand company-aware navigation](./understand-company-aware-navigation.md)

## Info

- App sections: `dashboard`, `reports`, `invoices`, `banking`, `customers`, `reconcile`, `vendors`, `bills`, `rules`, `ledger`, `checks`, `chart`, `checklists`, `companies`, `license`, `support`, `preferences`
- Last validated: 2026-05-01
- Screenshot status: `not-started`
