...
2. Choose the action that matches your goal:
   ...
   - `Import` menu then `Download Template` if you want a starter format before preparing your file.
3. For exports, review your current filters first because the export uses the rows visible in the ledger view.
4. For imports, select your file and review the preview modal before confirming.
5. If SPRK detects missing account labels during import, resolve them before final confirmation.
6. Confirm the import only after you have reviewed the preview totals, dates, descriptions, and accounts.
7. After import completes, refresh the ledger if needed and review the newly created entries.

## Expected Result

You can move journal-entry data in or out of the product with the current supported tools. Current general ledger impact as of 2026-05-02:
