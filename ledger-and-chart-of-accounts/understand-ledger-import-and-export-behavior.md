...
3. In the current `Import` start modal, review the template guidance before continuing:
   ...
   - visible template columns include `Entry #`, `Date`, `Entry Memo`, `Account`, `Description`, `Debit`, and `Credit`
...
7. Match `Account` labels to the accounts you intend to use.
...
8. If a source row omits `Date`, review the previewed date before confirming. The current import path can fill a default run-date value instead of failing immediately for a blank date.
9. If SPRK detects missing account labels during import, resolve or create the accounts through the import flow, then let SPRK re-preview the same file before commit.
...
- Assuming only raw account IDs work. Use the current template and review code-plus-name account labels in preview.
- Ignoring a blank source date just because the preview filled one. Confirm the defaulted date belongs in the intended posting period.
