...
7. Select `Reverse`.
8. Choose the posting date for the reversal:
...
9. Select `Reverse` only after confirming the reversal date and original entry lines.

![Reverse journal date choices](../screenshots/reconciliation/reverse-linked-journal-step-03-reverse-date-choice.png)

10. Review `Reconcile`, `Banking`, or `Ledger` to confirm the correction appears in the expected period.
