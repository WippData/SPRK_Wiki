...
4. When reversing an entry, choose the posting-date mode that fits the correction:
   ...
   - `original`
   - `custom`
5. Remember that a reversal creates a separate entry with flipped debit and credit amounts rather than deleting the original.
6. In `Chart of Accounts`, treat account deletion as deactivation. Prior activity remains part of the company history.
