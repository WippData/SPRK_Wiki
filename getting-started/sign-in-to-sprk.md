...
1. Open SPRK.
2. If onboarding has already been completed in this workspace, use the `Login` button on the `Local Only login` screen.
3. If onboarding has not been completed yet, choose the startup path that fits your goal:
   - `Start with Demo` creates a demo company and continues into the app.
   - `Import existing data` opens the import wizard from the startup flow.
   - `Skip` creates a blank company and continues into the app.
