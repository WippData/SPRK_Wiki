...
4. Use `Settings` -> `Defaults` when you want to store tenant-level defaults for future companies or bulk-apply required-account-field and edit-policy defaults to selected active companies.
5. Use the archive action to move a company out of the active list when you no longer want it used day to day.
6. If needed, switch to the `Archived` or `All` filter to review archived companies.
7. Unarchive a company if it needs to become active again.
8. Permanently delete a company only after it has been archived first.
9. If SPRK warns that the company has journal entries, keep it archived instead of expecting permanent delete to succeed.
