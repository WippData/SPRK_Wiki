...
1. Open `Checklist`.
2. Review the run table columns:
   - `Checklist`
   - `Period Type`
   - `Period Start`
   - `Period End`
   - `Status`
   - `Updated`
3. Use the `Filter` field when you want to search by checklist name, period type, notes, or status text.
4. Narrow the list further when needed:
   - Use the date controls for a time window.
   - Filter by `Status`.
   - Filter by `Period Type`.
5. Select `Clear` to reset the filters when you need the full list again.
6. Open a run from the table to review its stored responses and notes in detail.
